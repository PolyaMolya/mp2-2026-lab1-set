...
Структура проекта:
  ...
  - `labwork_bitfield` — директория с основными файлами.
    - `tbitfield.h` — заголовочный файл битового поля (реализации в `tbitfield.cpp`)
    - `tset.h` — заголовочный файл множества (реализации в `tset.h`)
  - `test_bitfield` — директория c Google Test.
...
В решении содержатся следующие модули:
  ...
  - Модуль `tbitfield`, содержащий реализацию класса битового поля (файлы
    `./tbitfield.h`, `./tbitfield.cpp`). Предполагается, что в ходе
    выполнения работы реализуются методы класса в файле `./tbitfield.cpp`,
    при этом заголовочный файл `./tbitfield.h` с объявлениями должен
    оставаться неизменным.
  - Модуль `tset`, содержащий реализацию класса множества (файлы
    `./tset.h`, `./tset.cpp`). При выполнении работы так же, как и в
    случае класса битового поля, разрабатывается только реализация методов
    класса.
  - Тесты для классов битовое поле и множество (файлы
    `./test_bitfield/test_tbitfield.cpp`, `./test_bitfield/test_tset.cpp`).
  - Пример использования класса битового поля и множества для поиска простых
    ...
    (файл `.labwork_bitfield/sample_prime_numbers.cpp`).
